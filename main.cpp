...
public:
    ...
    void updateHeightRec(PNod nod) {
        if (nod == NULL) return;
        nodeHeight(nod);
        updateHeightRec(nod->parent);
    }
    ...
    PNod deepest_node(PNod root) {
        PNod p = root, parent = NULL;
        while (p != NULL) {
            parent = p;
            int bf = p->balanceFactor;
            if (bf < 0)
                p = p->leftChild;
            else
                p = p->rightChild;
...
//    void predecesorKey(long long x, long long int &predecesor){
//        PNod search = NULL;
//        PNod parcurgere = NULL;
//        PNod parinte = NULL;
//        if (this->root == NULL)
//            return;
//
//        parinte = this->root;
//        while(parinte != NULL &&parinte->key != x){
//            parcurgere = parinte;
//            if (x < parinte->key)
//                parinte = parinte->leftChild;
//            else parinte = parcurgere->rightChild;
//        }
//
//        if (parcurgere != NULL) {
//
//            if (parcurgere->leftChild == NULL) {
//                PNod intoarcere;
//                intoarcere = parcurgere->parent;
//
//                while (intoarcere->parent != NULL && intoarcere->parent->leftChild != NULL &&
//                       intoarcere->parent->leftChild == intoarcere)
//                    intoarcere = intoarcere->parent;
//
//                if (intoarcere == this->root)
//                    return;
//                else predecesor = intoarcere->parent->key;
//            } else {
//                search = parcurgere->leftChild;
//                while (search->rightChild != NULL)
//                    search = search->rightChild;
//
//                predecesor = search->key;
//            }
//        }
//    }
//
//    void succesorKey(long long x, long long int &succesor){
//        PNod search = NULL;
//        PNod parcurgere = NULL;
//        PNod parinte = NULL;
//        if (this->root == NULL)
...
//
//        if (parcurgere != NULL) {
//        if (parcurgere->rightChild == NULL){
//            PNod intoarcere;
//            intoarcere = parcurgere->parent;
//
//            while(intoarcere->parent != NULL && intoarcere->parent->rightChild != NULL &&  intoarcere->parent->rightChild == intoarcere)
//                intoarcere = intoarcere->parent;
//
//            if (intoarcere == this->root)
//                return;
//            else succesor = intoarcere->parent->key;
//        }
//        else {
//            search = parcurgere->rightChild;
